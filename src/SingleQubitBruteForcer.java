public class SingleQubitBruteForcer {
	...
	public static void main(String[] args) {
		...
		// start1 = new Bloch(1, 0, 0);
		// target1 =
		// start1.T().H().Tt().H().T().H().T().H().Tt().H().T().H().T();
		// start2 = new Bloch(0, 1, 0);
		// target2 =
		// start2.T().H().Tt().H().T().H().T().H().Tt().H().T().H().T();
		// start3 = new Bloch(0, 0, 1);
		// target3 =
		// start3.T().H().Tt().H().T().H().T().H().Tt().H().T().H().T();
		//
		// start1 = new Bloch(1, 0, 0);
	...
	public static Path findRoute(int depth, Bloch cur1C, Bloch cur2C, Bloch cur3C, String pstep) {
		Path best = new Path(cur1C.dist(target1) + cur2C.dist(target2) + cur3C.dist(target3));
		if (depth > 0) {
			if (!pstep.equals("H")) {
				Path child = findRoute(depth - 1, cur1C.H(), cur2C.H(), cur3C.H(), "H");
				best.best(child);
			}
			if (!pstep.equals("T") && !pstep.equals("t") && !pstep.equals("Z")) {
				Path child = findRoute(depth - 1, cur1C.T(), cur2C.T(), cur3C.T(), "T");
				best.best(child);

				child = findRoute(depth - 1, cur1C.Tt(), cur2C.Tt(), cur3C.Tt(), "t");
				best.best(child);

				if (!pstep.equals("S") && !pstep.equals("s")) {
					child = findRoute(depth - 1, cur1C.S(), cur2C.S(), cur3C.S(), "S");
					best.best(child);

					child = findRoute(depth - 1, cur1C.St(), cur2C.St(), cur3C.St(), "s");
					best.best(child);

					child = findRoute(depth - 1, cur1C.Z(), cur2C.Z(), cur3C.Z(), "Z");
					best.best(child);
				}
			}
			if (!pstep.equals("X")) {
				Path child = findRoute(depth - 1, cur1C.X(), cur2C.X(), cur3C.X(), "X");
				best.best(child);
			}
			if (!pstep.equals("Y")) {
				Path child = findRoute(depth - 1, cur1C.Y(), cur2C.Y(), cur3C.Y(), "Y");
				best.best(child);
			}
		}
		best.add(pstep);
