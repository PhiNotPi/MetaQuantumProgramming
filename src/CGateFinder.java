public class CGateFinder {
	...
	public static Path findRoute(int depth, Bloch cur1, Bloch cur1C, Bloch cur2, Bloch cur2C, Bloch cur3, Bloch cur3C,
		...
		if (depth > 0) {
			...
			if (!pstep.equals("H")) {
				Path child = findRoute(depth - 1, cur1.H(), cur1C.H(), cur2.H(), cur2C.H(), cur3.H(), cur3C.H(), "H",
						maxC);
				best.best(child);
			}
			if (!pstep.equals("T") && !pstep.equals("t") && !pstep.equals("Z")) {
				Path child = findRoute(depth - 1, cur1.T(), cur1C.T(), cur2.T(), cur2C.T(), cur3.T(), cur3C.T(), "T",
						maxC);
				best.best(child);

				child = findRoute(depth - 1, cur1.Tt(), cur1C.Tt(), cur2.Tt(), cur2C.Tt(), cur3.Tt(), cur3C.Tt(), "t",
						maxC);
				best.best(child);
	...
	public static void findRouteG(int depth, Bloch cur1, Bloch cur1C, Bloch cur2, Bloch cur2C, Bloch cur3, Bloch cur3C,
		...
		if (depth > 0) {
			...
			if (!pstep.equals("T") && !pstep.equals("t") && !pstep.equals("Z")) {
				...
				findRouteG(depth - 1, cur1.Tt(), cur1C.Tt(), cur2.Tt(), cur2C.Tt(), cur3.Tt(), cur3C.Tt(), "t", maxC,
						path);
