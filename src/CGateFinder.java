public class CGateFinder {
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < 40; i++) {
      Path res = findRoute(i, start1, start1, start2, start2, start3, start3, "", 2);
      // findRouteG(i, start1, start1, start2, start2, start3, start3, "",
      // 3, "");
      // Path res = globalbest;
  ...
  public static Path findRoute(int depth, Bloch cur1, Bloch cur1C, Bloch cur2, Bloch cur2C, Bloch cur3, Bloch cur3C,
      String pstep, int maxC) {
    Path best = new Path(cur1.dist(start1) + cur1C.dist(target1) + cur2.dist(start2) + cur2C.dist(target2)
        + cur3.dist(start3) + cur3C.dist(target3));
    if (depth > 0) {
      if (!pstep.equals("C") && maxC > 0) {
        Path child = findRoute(depth - 1, cur1, cur1C.X(), cur2, cur2C.X(), cur3, cur3C.X(), "C", maxC - 1);
        best.best(child);
      }
      if (!pstep.equals("H")) {
        Path child = findRoute(depth - 1, cur1.H(), cur1C.H(), cur2.H(), cur2C.H(), cur3.H(), cur3C.H(), "H", maxC);
        best.best(child);
      }
      if (!pstep.equals("T") && !pstep.equals("t") && !pstep.equals("Z")) {
        Path child = findRoute(depth - 1, cur1.T(), cur1C.T(), cur2.T(), cur2C.T(), cur3.T(), cur3C.T(), "T", maxC);
        best.best(child);

        child = findRoute(depth - 1, cur1.Tt(), cur1C.Tt(), cur2.Tt(), cur2C.Tt(), cur3.Tt(), cur3C.Tt(), "t", maxC);
        best.best(child);
        ...
        // best.best(child);
        //
        // child = findRoute(depth - 1, cur1.St(), cur1C.St(),
        // cur2.St(),
  ...
  public static void findRouteG(int depth, Bloch cur1, Bloch cur1C, Bloch cur2, Bloch cur2C, Bloch cur3, Bloch cur3C,
      String pstep, int maxC, String path) {
    path += pstep;
    Path curpath = new Path(cur1.dist(start1) + cur1C.dist(target1) + cur2.dist(start2) + cur2C.dist(target2)
        + cur3.dist(start3) + cur3C.dist(target3));
    curpath.desc = path;
    globalbest.best(curpath);
    if (depth > 0) {
      if (!pstep.equals("C") && maxC > 0) {
        findRouteG(depth - 1, cur1, cur1C.X(), cur2, cur2C.X(), cur3, cur3C.X(), "C", maxC - 1, path);
      }
      if (!pstep.equals("H")) {
        findRouteG(depth - 1, cur1.H(), cur1C.H(), cur2.H(), cur2C.H(), cur3.H(), cur3C.H(), "H", maxC, path);
      }
      if (!pstep.equals("T") && !pstep.equals("t") && !pstep.equals("Z")) {
        findRouteG(depth - 1, cur1.T(), cur1C.T(), cur2.T(), cur2C.T(), cur3.T(), cur3C.T(), "T", maxC, path);

        findRouteG(depth - 1, cur1.Tt(), cur1C.Tt(), cur2.Tt(), cur2C.Tt(), cur3.Tt(), cur3C.Tt(), "t", maxC, path);
